...
function getCandidateTopReasons(cand, candidatePool) {
  if (!cand)
    return ["High foot traffic corridor", "Strong demographic demand coverage"];

  const pool =
    candidatePool && candidatePool.length > 1 ? candidatePool : [cand];
  ...
  const DRIVERS = [
    {
      ...
      higherIsBetter: true,
      label: (v, pctText) =>
        `High foot traffic -- ${Math.round(v).toLocaleString()} pedestrians/day (${pctText})`,
    },
    {
      field: "pop_density",
      higherIsBetter: true,
      label: (v, pctText) =>
        `Dense catchment population -- ${Math.round(v).toLocaleString()} nearby (${pctText})`,
    },
    {
      field: "nearby_metro_footfall",
      higherIsBetter: true,
      label: (v, pctText) =>
        `Strong metro/transit footfall -- ${Math.round(v).toLocaleString()}/day (${pctText})`,
    },
    {
      field: "commercial_activity",
      higherIsBetter: true,
      label: (v, pctText) =>
        `High commercial & retail activity index (${v.toFixed(1)}/100, ${pctText})`,
    },
    {
      ...
      higherIsBetter: true,
      label: (v, pctText) =>
        `Close to a major mall/market hub (proximity score ${v.toFixed(0)}/100, ${pctText})`,
    },
    {
      field: "dist_to_nearest_competitor",
      higherIsBetter: true, // farther from a competitor = less cannibalization
      label: (v, pctText) =>
        `Low competitor density -- nearest competitor ${v.toFixed(1)} km away (${pctText})`,
    },
    {
      field: "roi_index",
      higherIsBetter: true,
      label: (v, pctText) =>
        `Strong ROI index of ${v.toFixed(2)}x revenue-to-rent (${pctText})`,
    },
  ];

  const scored = DRIVERS.filter(
    (d) => cand[d.field] !== undefined && cand[d.field] !== null,
  )
    .map((d) => {
  ...
  if (scored.length === 0) {
    return [
      "Favorable ROI profile and quick payback",
      "Fills demographic coverage gap in zone",
    ];
  }
        ...
        ? {
            ...
            selected_candidates: (() => {
              const fullCandidatePool =
                ...
                [];
              return (window.lastOptimizeResult.selected_candidates || []).map(
                (cand) => {
                  return {
                    ...cand,
                    top_reasons: getCandidateTopReasons(
                      cand,
                      fullCandidatePool,
                    ),
                  };
  ...
  try {
    ...
    if (ownAtms && ownAtms.length > 0) {
      try {
        ...
        const avgUptime =
          ownAtms.reduce((sum, atm) => sum + (atm.uptime_pct || 0), 0) /
          ownAtms.length;
...
function updateModelsTable(metrics, bestModel) {
  ...
  // Render a fixed set of models so the UI always lists the three
  // algorithms that were evaluated historically. Fill metrics from
  // the payload when available, otherwise show placeholders.
  const modelList = ["XGBoost", "RandomForest", "LinearRegression"];

  // If no metrics object was passed, try falling back to global analytics
  if (!metrics && analyticsData && analyticsData.model_metrics) {
    metrics = analyticsData.model_metrics.metrics;
    bestModel = analyticsData.model_metrics.best_model_name;
  }

  // Populate rows for each expected model
  modelList.forEach((name) => {
    const m = metrics && metrics[name] ? metrics[name] : {};
    const isBest = name === bestModel;

    const r2 = m.r2_score !== undefined ? Number(m.r2_score).toFixed(4) : "--";
    const mae = m.mae !== undefined ? Number(m.mae).toFixed(2) : "--";
    ...
    tr.innerHTML = `
            ...
            <td style="padding: 10px; font-weight: 700; color: ${isBest ? "#16a34a" : "var(--text-primary)"};">${r2}</td>
    ...
    options: {
      ...
      scales: {
        x: {
          title: {
            display: true,
            text: "Actual Daily Transactions",
            font: { size: 10 },
          },
        ...
        y: {
          title: {
            display: true,
            text: "Predicted Daily Transactions",
            font: { size: 10 },
          },
          grid: { color: "rgba(0, 0, 0, 0.04)" },
          ticks: { color: "#475569", font: { size: 9 } },
...
function renderRiskCorrelationHeatmap() {
  ...
  cornerCell.style.cssText =
    "background: var(--bg-tertiary, #f1f5f9); border-radius: 4px; border: 1px solid var(--glass-border, #cbd5e1); display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 700; color: var(--text-muted, #64748b); padding: 4px;";
  ...
  features.forEach((feature) => {
    ...
    colHeader.style.cssText =
      "background: var(--bg-tertiary, #f1f5f9); border-radius: 4px; border: 1px solid var(--glass-border, #cbd5e1); padding: 6px 2px; text-align: center; font-size: 0.68rem; font-weight: 700; color: var(--text-primary, #1e293b); display: flex; align-items: center; justify-content: center; word-break: break-word; line-height: 1.15;";
  ...
  features.forEach((rowFeature) => {
    ...
    rowHeader.style.cssText =
      "background: var(--bg-tertiary, #f1f5f9); border-radius: 4px; border: 1px solid var(--glass-border, #cbd5e1); padding: 6px 8px; font-size: 0.70rem; font-weight: 700; color: var(--text-primary, #1e293b); display: flex; align-items: center; justify-content: flex-start; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;";
  ...
  legend.style.cssText =
    "display: flex; align-items: center; justify-content: center; gap: 1rem; margin-top: 0.5rem; font-size: 0.75rem; color: var(--text-secondary, #475569); font-weight: 600;";
  legend.innerHTML = `
